...
.face-down::after { content:'🂠'; font-size:26px; filter:drop-shadow(0 0 6px rgba(201,169,74,.4)); }
.card.empty-slot {
...
const MR     = {'♣':14,'♥':13,'♠':12,'♦':11}; // manilha suit rank
...
function handW(rounds) {
  ...
  if(n>=2){
    ...
    if(a===0&&b===0) return null;          // both draw → round 3
    if(a===0&&b!==0) return b;             // draw + win
    if(a!==0&&b===0) return a;             // win + draw
    if(a!==0&&b!==0&&a!==b) return null;  // split → round 3
  }
  if(n===3){
    if(w[0]===3) return 0;
...
function makeUrl(gs,p) {
  const base='https://sauloarradi.github.io/truco/';
  return base+'#g='+enc(gs)+'&p='+p;
}
...
function nextHand() {
  if(!GS) return;
  // Only the "dealer" (alternates each hand) can start — winner of previous hand starts next
  const seed=(Math.random()*999998+1)|0;
  const {p1,p2,vira}=deal(seed);
...
function renderSetup() {
  return `<div class="setup"><div class="setup-box">
    ...
    <div class="setup-sub">Jogue via WhatsApp · 2 jogadores</div>
    <label class="lbl">Seu nome (Jogador 1)</label>
    <input class="inp" id="in1" placeholder="Ex: João" autocomplete="off">
    <label class="lbl">Nome do amigo (Jogador 2)</label>
    <input class="inp" id="in2" placeholder="Ex: Maria" autocomplete="off">
    <button class="btn-start" id="btnStart">Criar Partida ↗</button>
  </div></div>`;
}
...
function renderGame() {
  ...
  // Header
  h+=`<div class="hdr">
    <div><div class="hdr-title">🃏 TRUCO</div><div class="hdr-hand">Mão vale ${pts}</div></div>
    <div><div class="score-n">${myScore} <span style="opacity:.35">×</span> ${oppScore}</div>
    <div class="score-lbl">${myName} &nbsp;×&nbsp; ${oppName}</div></div>
  </div>`;
  ...
  if(showRounds){
    ...
    gs.rounds.forEach((r,i)=>{
      ...
      let wlbl='';
      if(r.w===0)     wlbl=`<span class="drw">— Empate</span>`;
      else if(r.w===me) wlbl=`<span class="won">✓ Você</span>`;
      else             wlbl=`<span class="lost">✓ ${oppName}</span>`;
  ...
  h+=`<div class="panel">
    <div class="panel-lbl">${myName}<span class="you-tag">VOCÊ</span></div>
    <div class="cards-row">
      ${myCards.map((c,i)=>c
  ...
  if(gs.phase==='play'){
    if(itsMeTurn){
      h+=`<div class="turn-indicator" style="text-align:center;font-size:11px;color:var(--gold);letter-spacing:1px;font-style:italic;padding:2px 0">
        ✦ Sua vez — toque em uma carta para selecionar
      </div>`;
      ...
      h+=`<div class="wait-box">⏳ Aguardando <strong>${oppName}</strong> jogar...</div>`;
    }
  }
    ...
    if(gs.truco.by===me){
      ...
      h+=`<div class="truco-h">${oppName} pediu ${TLBL[gs.truco.vi]}</div>
          <div class="btn-row" style="flex-wrap:wrap;gap:6px">
            <button class="btn b-acc" id="btnAcc">✓ Aceitar (${TVALS[gs.truco.vi]} pts)</button>
            <button class="btn b-ref" id="btnRef">✗ Correr (${TVALS[gs.truco.vi-1]||1}pt p/ ${oppName})</button>
  ...
  if(gs.phase==='handEnd'){
    let title='';
    if(gs.hw===0)    title=`🤝 Empate! (+1 cada)`;
    else if(gs.hw===me) title=`✓ Você venceu a mão! (+${TVALS[gs.truco.vi]})`;
    else                title=`${oppName} venceu a mão (+${TVALS[gs.truco.vi]})`;
    h+=`<div class="end-wrap">
      <div class="end-h">${title}</div>
      <div class="end-sub">Placar: ${myScore} × ${oppScore}</div>
      <button class="btn b-next" id="btnNext">Próxima mão →</button>
    </div>`;
  }
  ...
  if(gs.phase==='gameOver'){
    ...
    h+=`<div class="end-wrap">
      <div style="font-size:40px;margin-bottom:8px">${iWon?'🏆':'👏'}</div>
      <div class="end-h" style="color:${iWon?'var(--gold)':'#fc8181'}">${iWon?'Você venceu o jogo!':oppName+' venceu o jogo!'}</div>
      <div class="end-sub">Placar final: ${gs.scores[me-1]} × ${gs.scores[opp-1]}</div>
      <button class="btn b-restart" id="btnRestart">🔄 Nova partida</button>
    </div>`;
  }
  ...
  if(LINK){
    ...
    if(needShare){
      let lbl=`📱 Envie para <strong>${oppName}</strong> jogar:`;
      if(gs.phase==='handEnd') lbl=`📱 Envie para <strong>${oppName}</strong> ver o resultado:`;
      if(gs.rounds.length===0&&me===1) lbl=`📱 Convide <strong>${oppName}</strong> para entrar na partida:`;

      const waMsg=encodeURIComponent(`${me===1&&gs.rounds.length===0?'Vamos jogar Truco! 🃏 Clique para entrar na partida':'Sua vez no Truco! 🃏'}
${LINK}`);
      ...
      h+=`<div class="share-box">
        ...
        <div class="share-row">
          ...
          <button class="b-copy ${COPIED?'ok':''}" id="btnCopy">${COPIED?'✓ Copiado!':'Copiar'}</button>
        </div>
